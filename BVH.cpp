...
template<typename T>
Octree<T>::Octree(const ObjectBoundingBox &boundingBox) {
    ...
    float dim = max(diff.x, max(diff.y, diff.z)) / 2;
...
template<typename T>
void Octree<T>::insert(OctreeNode<T> *node, const T *object, const Coord &boundMin,
...
template<typename T>
void
Octree<T>::computeChildBound(uint32_t index, const Coord &nodeCentroid, const Coord &boundMin, const Coord &boundMax,
...
template<typename T>
void Octree<T>::build(OctreeNode<T> *node, const Coord &boundMin, const Coord &boundMax) {
    if (node->m_isLeaf) {
        // update node bounding box
        for (auto object:node->m_data) {
            node->m_nodeBoundingBox.updateBoundingBox(object);
        }
    } else {
        for (int i = 0; i < 8; ++i) {
            if (node->m_child[i]) {
